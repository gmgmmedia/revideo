 ...
 * Duration: 08:00 - 15:00 (7 seconds)
...
export default makeScene2D('scene3', function* (view) {
  ...
  const HEX_CHARS = ['5', 'K', 'j', '2', 'v', 'X', '9', 'a', 'H', 'c', 'F', 'p'];
    ...
    <>
      ...
      <Node ref={heroBoxGroup} scale={0} opacity={0}>
        ...
        <Node ref={contentLayer} opacity={0}>
          {/* Content indicator lines with bits */}
          {Array.from({ length: 5 }, (_, i) => (
            <Node key={`content-row-${i}`}>
              <Line
                ref={contentLines}
                points={[
                  [-70, 65 + i * 14],
        ...
        <Node ref={dataFlowGroup} opacity={0}>
          {/* Flow lines showing data movement inside */}
          <Path
            ref={dataFlowLines}
            data="M -80,-60 Q -40,-20 0,0 Q 40,20 80,60"
            stroke={colors.neon}
            lineWidth={1}
            opacity={0}
            lineCap="round"
          ...
          <Path
            ref={dataFlowLines}
            data="M 80,-60 Q 40,-20 0,10 Q -40,40 -80,70"
            stroke={colors.neon}
            lineWidth={1}
            opacity={0}
            lineCap="round"
      ...
      <Node ref={comparisonGroup} opacity={0}>
        ...
        >
          ...
          <Node ref={execFunctionIcon} opacity={0}>
            <Circle size={8} fill={colors.neon} x={-5} y={35} />
            <Circle size={8} fill={colors.neon} x={5} y={35} />
            <Line
              points={[[-5, 35], [0, 45], [5, 35]]}
              stroke={colors.neon}
              lineWidth={2}
              lineCap="round"
            />
        ...
        {Array.from({ length: DATA_BOX_COUNT }, (_, i) => {
          ...
          return (
            ...
            >
              ...
              {/* Data type icon */}
              <Node ref={dataBoxIcons} y={-DATA_BOX_SIZE / 2 - 12} opacity={0}>
                <Circle size={16} fill={colors.background} stroke={colors.white} lineWidth={1} />
                <Txt
                  text={['$', '#', '%', '&', '@', '*'][i]}
                  fontFamily={fonts.data}
                  fontSize={10}
        ...
        {/* ===== Connecting Beziers (Program to Data) ===== */}
        {Array.from({ length: 3 }, (_, i) => {
          const startX = -280 + EXEC_SIZE / 2 + 20;
          const startY = 30 + (i - 1) * 40;
          const endX = 180 - DATA_BOX_SIZE / 2 - 15;
          const endY = -50 + i * (DATA_BOX_SIZE + 25);
  ...
  // --- Beat 17 (13:10) - Function icon appears ---
  yield* execFunctionIcon().opacity(1, timing.entrance);
  ...
  // --- Beat 19 (14:10) - Data box icons pop ---
  yield* sequence(
    0.03,
    ...dataBoxIcons.map((icon) => icon.opacity(0.7, timing.entrance))
  );
  ...
  // --- Beat 20.5 (14:25) - Animate particles along beziers ---
  // Particles move from program to data (simplified - actual path following would require more complex animation)
  yield* all(
    ...connectionParticles.map((particle, i) =>
      all(
  ...
  yield* all(
    ...connectionParticles.map((particle) => particle.opacity(0, timing.fast))
  );
